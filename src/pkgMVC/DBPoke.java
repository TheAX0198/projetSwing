package pkgMVC;

public interface DBPoke {

    public static final Pokemon[] LISTEPOKEMON = {

        new Pokemon("Arbok", "Sombre", 70, 130, "Il utilise la marque sur son ventre\npour intimider l’ennemi. Il étouffe\nl’ennemi pétrifié par la peur."),
        new Pokemon("Blastoise", "Eau", 220, 330, "Les trombes d’eau projetées par\nles canons de sa carapace peuvent\npercer le métal le plus résistant."),
        new Pokemon("Blaziken", "Feu", 210, 210, "Il peut bondir par-dessus un bâtiment de 30 étages.\nSes poings enflammés brûlent ses adversaires."),
        new Pokemon("Bulbasaur", "Plante", 20, 70, "Il a une étrange graine plantée sur son dos.\nElle grandit avec lui depuis sa naissance."),
        new Pokemon("Camerupt", "Feu", 120, 140, "Il porte des volcans sur son dos.\nQuand le magma remonte, il\nfrissonne et entre en éruption."),
        new Pokemon("Charmander", "Feu", 30, 60, "La flamme de sa queue symbolise sa\nvitalité. Elle est intense quand il\nest en bonne santé."),
        new Pokemon("Ditto", "Normal", 10, 170, "Il a la capacité de modifier sa\nstructure cellulaire pour prendre\nl’apparence de ce qu’il voit."),
        new Pokemon("Grovyle", "Plante", 40, 80, "Il vit dans les jungles denses.\nIl saute de branche en branche\npour approcher de sa proie."),
        new Pokemon("Magikarp", "Eau", 10, 30, "Magicarpe est un Pokémon ridicule qui ne sait faire que\ndes ronds dans l’eau ou se laisser porter par les courants.\nSon comportement a donné envie aux savants d’étudier\nson cas."),
        new Pokemon("Metagross", "Acier", 200, 330, "Il est né de la fusion de plusieurs\nMétang. Ses quatre cerveaux en\nfont l’égal d’un superordinateur."),
        new Pokemon("Mudkip", "Eau", 30, 70, "Il analyse le courant grâce à la\nnageoire sur sa tête. Il est assez\nfort pour soulever des rochers."),
        new Pokemon("Ninetales", "Feu", 80, 140, "On raconte que ses neuf queues\ndétiennent un pouvoir mystique.\nIl peut vivre pendant mille ans."),
        new Pokemon("Noivern", "Dragon", 140, 260, "Il peut littéralement pulvériser de gros rochers\ngrâce aux ultrasons émis par ses oreilles.\nIl surgit des ténèbres pour attaquer par surprise."),
        new Pokemon("Pikachu", "Électrique", 250, 310, "Il lui arrive de remettre d’aplomb\nun Pikachu allié en lui envoyant\nune décharge électrique."),
        new Pokemon("Serperior", "Plante", 120, 210, "Il ne donnera tout son potentiel que contre un\nennemi puissant indifférent à son regard écrasant\nde noblesse."),
        new Pokemon("Slowpoke", "Eau", 60, 80, "Bien que lent, c’est un pêcheur\nadroit qui utilise sa queue. Elle\nest insensible aux morsures."),
        new Pokemon("Spearow", "Normal", 40, 70, "Il fait battre vigoureusement ses\npetites ailes pour voler et cherche\nà manger dans l’herbe avec le bec."),
        new Pokemon("Squirtle", "Eau", 30, 70, "Il se réfugie dans sa carapace\net réplique en éclaboussant\nl’ennemi à la première occasion."),
        new Pokemon("Tangela", "Plante", 40, 80, "Ses lianes s’allongent au fil du temps.\nElles recouvrent tout son corps, dont on ignore\nla véritable forme."),
        new Pokemon("Zigzagoon", "Sombre", 30, 70, "Il marche en zigzaguant et n’a\npas son pareil pour trouver des\nobjets dans l’herbe ou sous terre.")
    };

}
